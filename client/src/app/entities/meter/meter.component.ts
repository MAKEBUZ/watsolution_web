  ...
  setup() {
    ...
    const retrieveMeters = async () => {
      ...
      try {
        ...
        const res = await meterService().retrieve(paginationQuery);
        totalItems.value = Number(res.headers['x-total-count']);
        queryCount.value = totalItems.value;
        links.value = dataUtils.parseLinks(res.headers?.link);
        meters.value.push(...(res.data ?? []));
      } catch (err) {
    ...
    onMounted(async () => {
      await retrieveMeters();
    });

    const removeId: Ref<number> = ref(null);
    const removeEntity = ref<any>(null);
    const prepareRemove = (instance: IMeter) => {
    ...
    const removeMeter = async () => {
      try {
        await meterService().delete(removeId.value);
        const message = t$('watsolutionApp.meter.deleted', { param: removeId.value }).toString();
    ...
    watch([meters, page], async ([data, page], [_prevData, prevPage]) => {
